...
{
    public abstract class ABPFrameworkTestBase : AbpIntegratedTestBase<ABPFrameworkTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ABPFrameworkTestBase()
        {
        ...
        protected void UsingDbContext(Action<ABPFrameworkDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABPFrameworkDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABPFrameworkDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABPFrameworkDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABPFrameworkDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPFrameworkDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABPFrameworkDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPFrameworkDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABPFrameworkDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABPFrameworkDbContext, Task<T>> func)
        {
            T result;
